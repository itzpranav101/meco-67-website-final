...
def handle_any_exception(e):
    # Always return JSON, even on a crash: the Meco server expects to be able to
    # parse every response as JSON, and a raw Flask HTML error page breaks that.
...
def add_cors_headers(resp):
    # Meco's Node server is the only intended caller; these headers just keep
    # direct local debugging (curl, a browser tab) usable.
...
# Unset (the default for local runs) leaves this service open on localhost, same
# as before. When deployed with a public URL, set this so only Meco's own Node
# server, which knows the same secret, can reach /enroll, /identify, etc.
...
def enroll():
    ...
    # One voiceprint per trusted person: re-recording replaces the previous sample.
    existing = None
    if person_id:
...
if __name__ == "__main__":
    ...
    # Render (and most hosts) assign PORT and expect the process to bind every
    # interface; local runs stay on loopback-only since nothing sets PORT there.
